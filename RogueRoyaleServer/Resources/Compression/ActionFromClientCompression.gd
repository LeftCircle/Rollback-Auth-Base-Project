...
func get_action_bitmap(action : ActionFromClient) -> int:
	var bitfield_int = 0
	for other_action in action.OTHER_ACTIONS:
		if action.is_action_pressed(other_action):
			bitfield_int = bitfield_int | action.get_action_binary_rep(other_action)
	return bitfield_int
