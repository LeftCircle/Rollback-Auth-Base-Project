...
func test_entities_queue_register_to_dodge_from_move() -> void:
	...
	var system_component : C_StateSystemState = character.get_component("StateSystem")
...
func test_entities_queue_register_to_move_if_no_stamina() -> void:
	...
	var stamina_component : ClientStamina = character.get_component("Stamina")
...
func test_dodge_component_update_is_tracked() -> void:
	var frame = random_frame()
	var character = await init_player_character()
	var dodge_component : AnimationDrivenDodge = character.get_component("Dodge")
	dodge_component.netcode.is_from_server = true
	var dodge_input : InputActions = _build_dodge_input()
	...
	assert_true(ComponentUpdateTracker.is_component_updated(next_frame, dodge_component))
...
func test_animation_tree_updates_with_dodge_execution() -> void:
	...
	var animation_tree : LocalRollbackAnimationTree = character.animation_tree
	assert_eq(animation_tree.get_animation_frame(), frames_to_execute)
	assert_eq(animation_tree.get_active_node(), "Dodge")

func test_rollback_resets_animation_to_correct_frame() -> void:
	# Run a few frames of the dodge system, cause a rollback to occur, and assert that the
	# animation player is reset to the correct frame.
	var frame = random_frame()
	var character = await init_player_character()
	var dodge_input : InputActions = _build_dodge_input()
	frame = register_input_and_execute_frame(frame, character, dodge_input)
	...
	var first_executed_frame = frame
	frame = register_input_and_execute_frame(frame, character, InputActions.new())
	assert_true(DodgeStateSystem.is_entity_registered(character))
	var dodge_component : AnimationDrivenDodge = character.get_component("Dodge")
	assert_eq(dodge_component.data_container.animation_frame, 1)
	# Assert the animation player is on the right frame too
	var animation_player : AnimationPlayer = character.animations
	var animation_tree : LocalRollbackAnimationTree = character.animation_tree
	assert_eq(animation_tree.get_animation_frame(), 1)

	var frames_to_execute = 5
	for i in range(frames_to_execute):
		frame = register_input_and_execute_frame(frame, character, InputActions.new())
	assert_eq(dodge_component.data_container.animation_frame, 1 + frames_to_execute)
	assert_eq(animation_tree.get_animation_frame(), 1 + frames_to_execute)

	# Trigger the rollback
	dodge_component.reset_to_frame(first_executed_frame)
	assert_eq(dodge_component.data_container.animation_frame, 1)
	assert_eq(animation_tree.get_animation_frame(), 1)
...
func test_entity_receives_inputs() -> void:
	var frame = random_frame()
	var character = await init_player_character()
	var dodge_input : InputActions = _build_dodge_held_input()
	frame = register_input_and_execute_frame(frame, character, dodge_input)
	var input_actions : InputActions = character.input_actions
	assert_true(input_actions.is_action_pressed("dodge"))
	assert_true(input_actions.previous_actions.is_action_pressed("dodge"))

func test_input_queue_cleared_on_entry() -> void:
	var frame = random_frame()
	var character = await init_player_character()
	var dodge_input : InputActions = _build_dodge_input()
	var input_queue : C_InputQueue = character.get_component("InputQueue")
...
func test_remote_character_also_enters_dodge_system() -> void:
	var frame = random_frame()
	var remote_char = await init_player_template()
	var dodge_input : InputActions = _build_dodge_input()
	frame = register_input_and_execute_frame(frame, remote_char, dodge_input)
	assert_true(DodgeStateSystem.is_entity_queued(remote_char))
	frame = register_input_and_execute_frame(frame, remote_char, InputActions.new())
	assert_true(DodgeStateSystem.is_entity_registered(remote_char))
