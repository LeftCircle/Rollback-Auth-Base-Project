...
class_name LocalRollbackAnimationTree
...
func _ready() -> void:
	...
	add_to_group("LocalRollbackAnimationTree")
	add_to_group("LocalHistory")
...
func save_history(frame : int) -> void:
	history.add_data(frame, state_machine_playback, current_blend_position)
...
func _on_rollback(frame : int) -> void:
	var data : RollbackAnimationStateMachineData = history.retrieve_data(CommandFrame.get_previous_frame(frame))
	start_playing_node_at_position(data.node_name, data.blend_position, data.current_animation_position)

func _on_tree_exiting() -> void:
	RollbackSystem.rollback_frame.disconnect(_on_rollback)
