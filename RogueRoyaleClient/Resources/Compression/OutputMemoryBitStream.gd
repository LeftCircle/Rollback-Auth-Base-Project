...
func gaffer_start_read(new_buffer : Array, bits_to_read : int) -> void:
	...
	if size % WORD_SIZE_IN_BYTES != 0:
		if size < WORD_SIZE_IN_BYTES:
			var new_array = []
			new_array.resize(WORD_SIZE_IN_BYTES - size)
			new_array.fill(0)
			mBuffer += new_array
		else:
			var new_array = []
			new_array.resize(WORD_SIZE_IN_BYTES - (size % WORD_SIZE_IN_BYTES))
			new_array.fill(0)
			mBuffer += new_array
	scratch = 0
	scratch_bits = 0
...
func get_array_to_send() -> Array:
	...
	var n_bytes_array = BaseCompression.compress_int_to_x_bytes(total_bits, BaseCompression.bytes_for_n_bits)
	byte_array.append_array(n_bytes_array)
	return byte_array
...
func is_finished() -> bool:
	assert(read_bits <= total_bits) #,"debug")
